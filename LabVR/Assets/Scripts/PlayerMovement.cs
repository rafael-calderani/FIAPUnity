...
public class PlayerMovement : MonoBehaviour {
	...
	private Vector3 endPoint = Vector3.up;
	...
	void Update() {
		...
		if (GvrControllerInput.TouchDown || Input.GetMouseButtonDown (0)) {
			if (Physics.Raycast(ray, out hit, distanceToMove)) {
				if (hit.transform.tag == "TargetToMove") {
					...
					endPoint = new Vector3(endPoint.x - 1.0f, endPoint.y + 1.0f, endPoint.z - 1.0f);
		...
		if (isMoving) {
			...
			this.transform.position = new Vector3(move.x, move.y, move.z); // desafio
